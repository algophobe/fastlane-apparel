...
import { PRODUCTS, getProductsByType, getProductTypes } from '@/data/products'
    ...
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 w-full">
          ...
          >
            ...
            <h1 className="font-display text-7xl md:text-9xl tracking-widest text-white leading-none mb-6">
              FASTLANE
              <br />
              <span className="text-[var(--brand-red)]">APPAREL</span>
            </h1>
            <p className="font-body text-[var(--text-secondary)] text-lg max-w-md">
              Built for the fast lane. Premium streetwear that moves with you.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="border-t border-[var(--border)] py-20">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="font-display text-4xl tracking-widest text-white mb-2">PAY YOUR WAY</h2>
            <p className="text-[var(--text-secondary)] font-body">
              Cash App · Venmo · Zelle — no account required
            </p>
          </div>
          <div className="flex items-center gap-3">
            {['Cash App', 'Venmo', 'Zelle'].map(p => (
              <span key={p} className="text-xs font-mono text-[var(--text-muted)] bg-[var(--surface-3)] border border-[var(--border)] px-3 py-1.5 rounded-full">
                {p}
              </span>
            ))}
          </div>
